...
- 이 책에서 제공하는 예제 탬플릿을 사용하면 상관관계를 쉽게 파악할 수 있음  
...
ex) nll 값을 허용하지 않게 스키마를 정의했지만 null이 들어온 경우 트랜스포메이션은 오류 발생
...
그러나 아마존 S3 같이 궁극적 일관성 방식을 사영하는 클라우드 서비스를 저장소로 사용하면 중복 데이터가 발생할 수 있음  
