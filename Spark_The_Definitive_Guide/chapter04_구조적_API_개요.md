...
- Spark의 DataFrame은 Row 타입으로 구성된 Dataset인데,  
  Row 타입은 <b>Spark가 사용하는 '연산에 최적화된 인메모리 포멧'의 내부적인 표현 방식</b>
...
- runtime vs compile time
  - 프로그램을 생성하기 위해 개발자는 첫째로 소스코드를 작성하고 컴파일이라는 과정을 통해 기계어코드로 변환 되어 실행 가능한 프로그램이 되며  
    이러한 과정을 <b>컴파일타임</b> 이라고 함
  - 컴파일 과정을 마친 프로그램은 사용자에 의해 실행되어지며, 이런 응용프로그램이 실행되는 시점을 런타임이라고 부름  
    런타임과 컴파일타임은 두 가지 타입의 에러를 구분짓기 위하여 구분됨 
      - 컴파일타임 타입의 오류  
        syntax error  
        type error
      - 런타임 타입의 오류  
